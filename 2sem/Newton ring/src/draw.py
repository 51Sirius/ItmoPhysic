...
def draw_plots(Lambda, Lamda_q, Spectr, Radius):
    ...
    I_mono_array_r = np.array([intens_mono(wavelenght=Lambda, r=radius, Radial=Radius) for radius in r_array])
    I_quasi_array_r = np.array([intens_quasimono(Lamda_q, Spectr, r=radius, Radial=Radius) for radius in r_array])
